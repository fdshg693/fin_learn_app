...
- [ ] **Step 1: 現状の全テストがグリーンであることを確認（ベースライン）**

Run: `dotnet test`
Expected: PASS（全件）。この後の変更で挙動が変わらないことの基準にする。

- [ ] **Step 2: `Microsoft.Identity.Web` の最新 3.x 安定版バージョンを確認**
...
- [ ] **Step 6: `appsettings.Development.json` に AzureAd セクションを追加**
...
- [ ] **Step 7: 全テストが依然グリーンであることを確認（挙動不変の検証）**
