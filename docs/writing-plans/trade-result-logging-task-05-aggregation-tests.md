# Task 5: TurnResult の集計仕様を検証する新規テスト

親プラン: [trade-result-logging.md](./trade-result-logging.md)

**Files:**
- Create: `tests/FinLearn.Tests/TurnProcessorLoggingTests.cs`

仕様 §5.4 のエッジケース表を網羅するためのテストを追加。

- [ ] **Step 1: 失敗するテストを書く**

`tests/FinLearn.Tests/TurnProcessorLoggingTests.cs` を新規作成:
...
public class TurnProcessorLoggingTests
{
    ...
    public void Buy成行約定ゼロ_はFillsが空でSubmittedOrdersはコンピューターのみ()
    {
        ...
        // NoOpOrderPlacer はコンピューター注文を出さないが、プレイヤー注文は提出されている
...
Run: `dotnet test tests/FinLearn.Tests/FinLearn.Tests.csproj --filter "FullyQualifiedName~TurnProcessorLoggingTests"`
...
```bash
git add tests/FinLearn.Tests/TurnProcessorLoggingTests.cs
git commit -m "test(core): cover TurnResult aggregation per spec edge cases"
```
