...
毎ターン、プレイヤー注文の**前**に `ComputerTrader.PlaceOrders` が呼ばれ、20件の指値注文を生成する。
...
買い価格帯（85〜105%）と売り価格帯（95〜115%）が重なるため、同一ターン内でもコンピューター同士の約定が発生する。
