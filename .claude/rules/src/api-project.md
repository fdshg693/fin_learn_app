...
- **認証/認可 (Entra ID)**: `/api/games`・`/api/admin` のルートグループに `RequireAuthorization("ApiScope")` を適用。`ApiScope` は名前付きポリシーで「認証済み + scope `access_as_user`」を要求（`Microsoft.Identity.Web` の JWT Bearer 検証）。グローバルフォールバックポリシーは設定しないため `/`・`/play`・静的ファイルは未保護のまま。`AzureAd` 設定は公開情報のみ（本番は環境変数 `AzureAd__*` で上書き）。テストは `AuthTestWebApplicationFactory` がテスト用スキームを既定に差し替え、`TestAuthHandler` がヘッダ `X-Test-Auth`（既定=認証+scp、`none`=未認証、`noscope`=scp なし）で挙動を切替。詳細は [docs/AUTH.md](../../../docs/AUTH.md)
